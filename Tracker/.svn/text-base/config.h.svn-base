#pragma once

#include <config_base.h>

class Cconfig: public Cconfig_base
{
public:
	typedef std::set<int> t_listen_ipas;
	typedef std::set<int> t_listen_ports;

	int set(const std::string& name, const std::string& value);
	int set(const std::string& name, int value);
	int set(const std::string& name, bool value);
	// ** Construct an object with the default configuration
	Cconfig();
	// ** Construct an object with the configuration from another Cconfig object
	Cconfig(const Cconfig&);
	const Cconfig& operator=(const Cconfig&);

	// ** Configuration variables
	bool m_anonymous_announce;
	bool m_anonymous_connect;
	bool m_anonymous_scrape;
	bool m_auto_register;
	bool m_daemon;
	bool m_debug;
	bool m_full_scrape;
	bool m_gzip_debug;
	bool m_gzip_scrape;
	bool m_log_access;
	bool m_log_announce;
	bool m_log_scrape;
	int m_announce_interval;
	int m_clean_up_interval;
	int m_read_config_interval;
	int m_read_db_interval;
	int m_scrape_interval;
	int m_write_db_interval;

	//*_ TSUP parameters
	int m_hello_interval;		//!< interval in seconds in which a HELLO packet is send to another tracker
	int m_reconnect_interval;	//!< interval in seconds in which a SYN packet is send to another tracker that is not virtually connected
	int m_update_interval;		//!< interval in seconds in which an UPDATE packet is send to another tracker if there are updates available
	int m_disconnect_interval;	//!< interval in seconds in which no packet is received from another tracker and the virtual connection with it is closed
	int m_mandate_interval;		//!< interval in seconds in which the mandates of the swarm leaders takes place
	int m_max_delay_interval;	//!< maximum interval in seconds which are waited before restarting a new three-way-handshake
	int m_retry_times;			//!< number of times to retry sending a SYN packet at hello_interval instead of reconnect_interval
	std::string m_ip;
	std::string m_tracker_name;
	std::string m_table_trackers;

	std::string m_column_files_completed;
	std::string m_column_files_fid;
	std::string m_column_files_leechers;
	std::string m_column_files_seeders;
	std::string m_column_users_uid;
	std::string m_mysql_database;
	std::string m_mysql_host;
	std::string m_mysql_password;
	std::string m_mysql_table_prefix;
	std::string m_mysql_user;
	std::string m_offline_message;
	std::string m_query_log;
	std::string m_pid_file;
	std::string m_redirect_url;
	std::string m_table_announce_log;
	std::string m_table_deny_from_hosts;
	std::string m_table_files;
	std::string m_table_files_users;
	std::string m_table_scrape_log;
	std::string m_table_users;
	std::string m_torrent_pass_private_key;
	t_listen_ipas m_listen_ipas;
	t_listen_ports m_listen_ports;
private:
	// ** Sets the configuration variables from the Cconfig object sent as parameter (if NULL, defaults are used) and populates the maps m_attributes_*
	void fill_maps(const Cconfig*);
};
