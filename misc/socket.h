...
public:
	...
	static in_addr_t get_host(const std::string& name); //!<Returns the in_addr_t of the host name, which is in the internet dotted notation or is a domain name. Replaces original function.
	static std::string inet_ntoa(int h);
	static int start_up();
	static void make_sockaddr_in(sockaddr_in &sa, in_addr_t address, int port);
