...
/**
 *_ Returns the in_addr_t of the host name, which is in the Internet dotted notation or is a domain name. Replaces original function.
 */
in_addr_t Csocket::get_host(const std::string& name)
{
	in_addr_t addr;

	// Internet dotted notation
	addr = inet_addr(name.c_str());

	// Domain name
	if(addr == (in_addr_t)-1)
	{
		hostent *e = gethostbyname(name.c_str());
		if(e && e->h_addrtype == AF_INET && e->h_length == sizeof(in_addr) && e->h_addr_list)
			addr = *reinterpret_cast<int*>(*e->h_addr_list);
		else
			addr = INADDR_NONE;
	}
...
/**
 * Puts into 'sa' a socket address with the IP 'address' and the port 'port'.
 */
void Csocket::make_sockaddr_in(sockaddr_in &sa, in_addr_t address, int port)
{
